...
namespace selfdefined_controllers
{
  ...
  protected:
    ...
    // ---------------------自定义功能函数----------------------------------
    bool updateTrajectoryCommand(const JointTrajectoryConstPtr &msg,
                                 RealtimeGoalHandlePtr gh,
                                 std::string *error_string = nullptr);
    // 从"/command"话题中获取需要执行的轨迹
    void trajectoryCommandCB(const JointTrajectoryConstPtr &msg);
    void dualtrajectoryCommandCB(const JointTrajectoryConstPtr &msg);
    void goalCB(GoalHandle gh);
    // 处理动作目标的取消请求
    void cancelCB(GoalHandle gh);
    // 中止当前的活动目标（ActiveGoal）
    void preemptActiveGoal();
    // 查询轨迹状态的服务请求
    bool queryStateService(control_msgs::QueryTrajectoryState::Request &req,
                           control_msgs::QueryTrajectoryState::Response &resp);
    void targetWrenchCB(const geometry_msgs::WrenchStampedConstPtr &msg);
    void currentWrenchCB(const geometry_msgs::WrenchStampedConstPtr &msg);
    void currentRelerrorposCB(const geometry_msgs::PoseStampedConstPtr &msg);
    // ---------------------自定义功能函数----------------------------------
    ...
    // 获取父节点
    std::string getParentNamespace(const ros::NodeHandle &nh, int level = 1);
    trajectory_msgs::JointTrajectoryPtr processTrajectory
      (const trajectory_msgs::JointTrajectoryConstPtr &msg, bool use_last_six);
  private:
    ...
    ros::Subscriber currel_errorpose_sub_;
    ros::Subscriber dual_trajectory_command_sub_;
    // 目标wrench 和 当前wrench 的误差
    Vector6D error_wrench_;
    // 当前wrench
    pinocchio::Force current_wrench_;
    pinocchio::SE3 currel_errorpose_;
    // 目标wrench
    pinocchio::Force target_wrench_;
    // 实时buffer
    realtime_tools::RealtimeBuffer<Vector6D> error_wrench_rb_;
    realtime_tools::RealtimeBuffer<pinocchio::Data> pino_rstate_rb_;
    realtime_tools::RealtimeBuffer<pinocchio::SE3> currel_errorpose_rb_;
    // ------------自定义功能函数参数----------------
  };
} // namespace selfdefined_controllers
